...
def generate_wind_distribution_product(tiles, m64rn4, nb_classes = 36, shape = (44, 44, 1)):
    ...
    tiles_stacked_no_nan = tiles_stacked_no_nan.assign(wind_direction_distribution = (['all_tiles', 'bin_centers'],  predictions))

    # Compute mean wind direction, most likely wind direction and associated standard deviation 
    mean_wdir = xr.apply_ufunc(compute_mean_direction, tiles_stacked_no_nan.wind_direction_distribution, tiles_stacked_no_nan.bin_centers,
                               input_core_dims=[['bin_centers'], ['bin_centers']],
                               vectorize=True
                               )
    
    most_likely_wdir = xr.apply_ufunc(compute_most_likely_direction, tiles_stacked_no_nan.wind_direction_distribution, tiles_stacked_no_nan.bin_centers,
                                      input_core_dims=[['bin_centers'], ['bin_centers']],
                                      vectorize=True
                                      )

    std_wdir = xr.apply_ufunc(compute_standard_deviation, tiles_stacked_no_nan.wind_direction_distribution, tiles_stacked_no_nan.bin_centers,
                              input_core_dims=[['bin_centers'], ['bin_centers']],
                              vectorize=True
                              )
    ...
    tiles_stacked_no_nan['most_likely_wdir'].attrs['definition'] = '180° ambiguous most likely wind direction, defined with a bin precision of 2.5°.'
